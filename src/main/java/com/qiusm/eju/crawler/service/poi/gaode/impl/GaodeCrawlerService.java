...
import static com.qiusm.eju.crawler.constant.poi.GaodeUrl.CITY_POI_URL;
...
public class GaodeCrawlerService {
    ...
    public void allCityFenceStart() {
        List<GaodeCityPoiInfo> var1List = gaodeService.selectAllCityInfo();
    ...
    public void cityFenceStart(String cityName) {
        ...
        entityWrapper.eq("name", cityName);
        List<GaodeCityPoiInfo> var3 = gaodeService.selectCityInfoExample(entityWrapper);
    ...
    public void cuttingAllCityElectronicFence(String cityName) {
        EntityWrapper<GaodeCityFence> entityWrapper = new EntityWrapper<>();
        entityWrapper.eq("name", cityName);
        List<GaodeCityFence> cityFences = gaodeService.selectCityFenceByExample(entityWrapper);
                ...
                fixedThreadPool.execute(() -> {
                    try {
                        ...
                        for (String fenceStr : fenceArr) {
                            ...
                            gaodeService.saveCityPoint(cityPoints);
                        }
     ...
     *
     * @param cityName 城市名称
     * @param tagMap   标签集合：key是标签编码，value是标签中文含义。参见{@link GaodePOIType}
     */
    public void callCityPoiInfo(String cityName, Map<String, String> tagMap) {
        // 根据城市查询城市的POI点
        EntityWrapper<GaodeCityPoint> entityWrapper = new EntityWrapper<>();
        entityWrapper.eq("name", cityName);
        List<GaodeCityPoint> cityPoints = gaodeService.selectCityPointByExample(entityWrapper);
                    ...
                    try {
                        tagMap.forEach((tagCode, tagName) -> {
                            ...
                            if (pois.size() > 0) {
                                // 表示处理成功了
                                point.setStatus("ok");
                                gaodeService.updateCityPointByKey(point);
                                gaodeService.savePoi(pois);
                            }
                        });
                    } catch (Exception e) {
                        e.printStackTrace();
    ...
    private void loadCityFence(GaodeCityPoiInfo cityPoiInfo) {
        try {
            ...
            if (StringUtils.isNotBlank(body)) {
                ...
                gaodeService.saveCityFence(cityPoiInfo, fence);
